...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Lens.org');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'patents_search',
    description: 'Search Lens.org patent database by free-text or Lucene query (requires Bearer token); returns up to 1000 patent records with title, applicant, filing date, and claims.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Search Lens.org scholarly works (academic papers, preprints) by free-text or Lucene query (requires Bearer token); returns title, authors, journal, year, and DOI.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string' },
  ...
  { name: 'patent', description: 'Fetch a single Lens.org patent record by its lens_id (requires Bearer token); returns full bibliographic data including claims, inventors, and priority dates.', inputSchema: { type: 'object', properties: { lens_id: { type: 'string' } }, required: ['lens_id'] } },
  { name: 'scholarly', description: 'Fetch a single Lens.org scholarly work by its lens_id (requires Bearer token); returns full metadata including abstract, authors, citations, and open-access links.', inputSchema: { type: 'object', properties: { lens_id: { type: 'string' } }, required: ['lens_id'] } },
];

async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  const apiKey = (args._apiKey as string | undefined)?.trim();
  if (!apiKey) throw new Error('Lens.org requires a Bearer token: pass your key as the _apiKey argument (free for academic use at https://www.lens.org/lens/user/subscriptions).');
